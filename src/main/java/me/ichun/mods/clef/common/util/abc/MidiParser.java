...
public class MidiParser
{
    ...
    public static TrackInfo parse(File file)
    {
        ...
        try
        {
            ...
            // Add an initial node with the ticksPerBeat to set it (as the ticksPerBeat may not end up set otherwise)
            ...
            for (Track track : sequence.getTracks())
            {
                ...
                for (int i = 0; i < track.size(); i++)
                {
                    ...
                    if (midiEvent.getMessage() instanceof MetaMessage) //contains metainformation such as tempo, tune, etc
                    {
                        parseMetaMessage(midiEvent, trackNotes, trackInfo, ticksPerBeat);
                    } else if (midiEvent.getMessage() instanceof ShortMessage) // contains the actual notes
                    {
                        parseShortMessage(midiEvent, trackNotes, track, i);
                    } else
                    {
        ...
        {
            Clef.LOGGER.warn("Error reading MIDI file: " + file.getName(), e);
        }
        return null;
    }

    private static void parseShortMessage(MidiEvent midiEvent, TreeMap<Long, Set<Note>> trackNotes, Track track, int i)
    {
        ...
        switch (message.getCommand())
        {
            case ShortMessage.NOTE_ON:
            {
                ...
                note.volume = MathHelper.clamp(velocity / 128F, 0.1F, 0.9F);
                ...
                for (int j = i + 1; j < track.size(); j++)
                {
                    ...
                    if (possibleEnd.getMessage() instanceof ShortMessage && ((ShortMessage) possibleEnd.getMessage()).getCommand() == ShortMessage.NOTE_OFF)
                    {
                        ...
                        {
                            // this is the end of that note. Calculate the duration based of that
                ...
                {
                    LOGGER.warn("Uh oh, we could not find the end for a key!"); // That should not happen.
                }
            }
            break;
            case ShortMessage.NOTE_OFF:
            {
                // We don't really care about this case - we immediately search for NOTE_OFF when finding a NOTE_ON
//                                int key = message.getData1();
//                                int velocity = message.getData2();
            }
            break;
    ...
    private static void parseMetaMessage(MidiEvent midiEvent, TreeMap<Long, Set<Note>> trackNotes, TrackInfo trackInfo, int ticksPerBeat)
    {
